...
class ArucoFollower:
    def __init__(self):
        rospy.init_node('aruco_follower_node', anonymous=True)
        ...
        self.target_id = 42           
        self.marker_size = 0.08       # Real world size of the tag to measure in meters
        ...
        self.kp_y = 0.6   # Up/Down gain
        self.kp_yaw = 0.8 # Rotation gain
        ...
        if ids is not None:
            for i in range(len(ids)): # Normally there will be only one marker, but in case
                if ids[i][0] == self.target_id:
                    ...
                    # Preparing cmd vel
                    self.velocity_msg.linear.x = self.limit_speed(error_forward * self.kp_x)
...
if __name__ == '__main__':
    try:
        ...
        follower.takeoff_pub.publish(Empty())
        rospy.spin()
